...
stepLen = 0.5;              % desired step length in meters
theta = acosd(stepLen/2);   % in degrees
q1d = -170;                 % pre-impact velocity in deg/s
q3f = 170;                  % torso angle at impact in deg
q3df = 5;
a3 = 5;   
g3 = 165;
...
% Bezier coefficients
alpha = [a3, a4, a5]*pi/180;    %for q2 - alpha 3rd - 5th
gamma = [g3, g4, g5]*pi/180;    %for q3 - alpha 3rd - 5th
...
ub(1) = min(ub(1), -0.05);   % q1^- must stay negative
ub(2) = min(ub(2), -0.3);    % dq1^- must stay negative
...
% Compute and display Cost of Transport
[r,m_leg,Mh,Mt,l,g_val] = func_model_params;
Mtot = 2*m_leg + Mh + Mt;
step_length = abs(2 * r * sin(f(1)));
CoT = J / (Mtot * g_val * step_length);
disp(['Step length = ', num2str(step_length), ' m']);
