...
f = [-0.4019, -2.2188, 0.1029, 0.5129, 0.6019, 2.9669, 2.8963, 3.1167];
...
%
    function [limits,terminal,direction] = event(~,x)
        ...
        if pm2(1) > pm1(1) && vcm(2) < 0
            limits = P2(2);
        else
            limits = 1;   % disable event until geometry is valid
        end
